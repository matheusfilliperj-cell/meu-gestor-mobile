...
st.title("Oi!, [nome]! Tudo certo? Aqui é o Matheus Scheid, consultor da Direcional. Conferi aqui no sistema que se interessou em adqurir um imóvel com a gente, conseguiu? Eu consigo te ajudar com boas condições promocionais agora. O ideal é a gente marcar uma visita pra tu conhecer e já sair com uma simulação pronta de forma totalmente gratuita e sem compromisso. Estaria livre [dia]?”
👉 Se não: “Tranquilo, então te mando tudo pelo WhatsApp pra adiantar, podemos adiantar a simulação de forma completante online, o que acha?. Posso te chamar por lá?”")
...
if arquivo:
    ...
    if p < len(df):
        ...
        with col2:
            if st.button("🟥 DESCARTADO", use_container_width=True):
